  ...
  const renderMessageWithMentions = (text: string): React.ReactNode => {
    ...
    while ((match = mentionPattern.exec(text)) !== null) {
      ...
      const mentionedUser = users.find(user => 
        user.name === mentionedUsername ||
        user.name.toLowerCase() === mentionedUsername.toLowerCase()
      );
      ...
      parts.push(
        <span
          ...
          title={mentionedUser ? `${mentionedUser.name} (${mentionedUser.role})` : mentionedUsername}
        >
          @{mentionedUsername}
        </span>
      );
