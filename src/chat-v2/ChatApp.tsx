...
const ChatMain: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (formData: MessageFormData) => {
    ...
    const optimisticMessage: ChatMessage = {
      ...
      timestamp: new Date(), // Use Date object for consistency with chatService conversion
      type: 'message',
      isDeleted: false,
      status: 'sending',
      localId,
    ...
    try {
      ...
      if (result.error) {
        // 送信失敗
        dispatch(failOptimisticMessage(localId));
        console.error('Failed to send message:', result.error);
      } else if (result.id) {
        // 送信成功 - リアルタイム監視で実際のメッセージが届くので、ここでは何もしない
        // メッセージの重複を避けるため、楽観的メッセージはリアルタイム更新で置き換わる
      }
    } catch (error) {
      dispatch(failOptimisticMessage(localId));
