...
interface VirtualMessageListProps {
  ...
  messagesEndRef?: React.RefObject<HTMLDivElement | null>; // スクロール用参照を追加
  ...
  onRetryMessage: (message: ChatMessage) => void; // 再送信関数
...
export const VirtualMessageList: React.FC<VirtualMessageListProps> = ({
  ...
  // メッセージの安定化のためのmemoization
  const stableMessages = useMemo(() => {
    return messages
      .filter(msg => msg && msg.id) // 無効なメッセージを除外
      .reverse(); // 最新メッセージが下に表示されるように逆順に
  }, [messages]);
  ...
  // 新しいメッセージが追加されたときの自動スクロール（改善）
  useEffect(() => {
    if (messages.length > 0) {
      // 少し遅延をおいてスクロールすることで、レンダリング完了を待つ
      const timeoutId = setTimeout(() => {
        if (parentRef.current && !scrollingRef.current) {
          // より確実に最下部にスクロール
            ...
            >
              <div
                className={`flex space-x-4 group px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-700/50 relative transition-colors ${getPriorityColor(message.priority)} ${
                  ...
                  message.status === 'failed' ? 'opacity-80 bg-red-50/30 dark:bg-red-900/10' :
                  ''
                }`}
              >
                {/* アバター */}
                    ...
                    <div className="text-gray-800 dark:text-slate-200 break-words">
                      {message.isDeleted ? (
                        ...
                        <>
                          ...
                          />
                          {/* 送信失敗時の再送信ボタン */}
                          {message.status === 'failed' && message.authorId === currentUser?.userId && (
