...
const DiscordLikeChat = () => {
  ...
  const handleRetryMessage = useCallback(async (message: ChatMessage) => {
    ...
    if (message.status === 'failed') {
      ...
      // （useChat内で同じ処理が行われるため、新しい楽観的メッセージとして再送信）
      await sendNewMessage(message.content, message.attachments);
    }
  }, [sendNewMessage]);
  ...
  const handleCreateChannel = useCallback(async (channelData: any) => {
    try {
      ...
      if (channelId) {
        ...
        // 少し待ってから選択（Firebaseの同期を待つ）
        setTimeout(() => {
          console.log('🔔 チャンネルを選択しようとしています:', channelId);
          selectChannel(channelId);
        }, 1000);
      } else {
        console.error('❌ チャンネル作成に失敗しました');
        alert('チャンネルの作成に失敗しました。コンソールを確認してください。');
      }
    } catch (error) {
      console.error('💥 チャンネル作成エラー:', error);
      alert('チャンネル作成中にエラーが発生しました。');
    }
  ...
  useEffect(() => {
    ...
    initData();
  }, [channels.length, isLoading, currentUser?.userId]); // userIdのみを依存配列に
  ...
  }, [messages.length]); // メッセージの数が変わったときのみ実行
